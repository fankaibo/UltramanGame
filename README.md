...
这是 **M0 可运行开发原型**，已经在本机 Unity 6000.6.0f1 编译、构建并打开游戏，使用合成动作通过真实 Python → Unity 链路完成一局战斗。前置摄像头采集和原生姿态推理也已通过短时测试。英雄目前是为验证动作制作的红紫银配色简模，迪迦正式造型、完整变身演出和美术资源仍需继续完成。

Python、C# 规则和回环通信共 63 项自动检查通过，包括人在画面内但手肘被遮挡时的回归检查。中文字体随游戏内置，已经核对实际窗口中的中文、变身、攻击反馈和胜利画面。真人动作准确率和电视试玩仍须单独验证，详见 [验证记录](docs/验证记录.md)。
...
站到肩膀、手肘、双手都能入镜的位置，双手举高并保持片刻完成变身。挥拳前先收手。按 `F11` 切换全屏，按 `Esc` 暂停，按 `R` 重来。摄像头模式与键盘练习可用 `F2` 切换。
