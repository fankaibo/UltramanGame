...
namespace UltramanGame.Runtime
{
    public sealed class ArenaController : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            font=Font.CreateDynamicFontFromOSFont(new[] { "PingFang SC","Microsoft YaHei","Arial" },26);
        ...
        void CreateScene()
        {
            ...
            RenderSettings.ambientLight=new Color(.47f,.56f,.72f);
        ...
        void Update()
        {
            ...
            else
            {
                ...
                if(!PoseQuality.Valid(pose,now)) { input=default;recognizer.Reset(); }
                held=input;
            }
            ...
            hero.Update(!keyboard && input.Tracking?pose:null,battle,dt,Time.time);
            enemy.Update(null,battle,dt,Time.time);
            hero.Root.rotation=Quaternion.Slerp(hero.Root.rotation,
                Quaternion.Euler(0,battle.Phase==GamePhase.Transforming||battle.Phase==GamePhase.Victory?150:0,0),dt*3);
        ...
        void Restart()
        { battle=new Battle();recognizer.Reset();pose=null;held=default;paused=false;enemy.Root.localScale=Vector3.one;lastHealth=Battle.MaxHealth;caption="双手举高，准备变身";voice.Stop(); }
        ...
        void OnGUI()
        {
            ...
            GUI.Label(new Rect(1090,22,150,40),keyboard?"键盘练习":"摄像头体感",small);
            ...
            {
                Rect(330,215,650,210,new Color(.035f,.075f,.14f,.92f));
                string text=battle.Phase==GamePhase.Waiting?(keyboard?"按空格，变身！":"双手举过肩膀，准备变身"):
                    battle.Phase==GamePhase.Transforming?"光的力量，正在苏醒":
                    battle.Phase==GamePhase.Victory?"你守护了这座城市！":paused?"休息一下 · 点击继续":"回到摄像头前，准备继续";
                GUI.Label(new Rect(350,235,610,80),text,body);
                if(battle.Phase==GamePhase.Waiting)
                    GUI.Label(new Rect(375,330,560,65),keyboard?"A / D 挥拳    按住 S 防御    J 光线":"让肩膀、手肘和双手进入画面\n启动摄像头服务后，英雄会跟着你抬手",body);
            ...
            GUI.Label(new Rect(25,668,640,38),keyboard?"空格变身 · A/D挥拳 · S防御 · J光线 · Esc暂停":PoseQuality.Valid(pose,DateTimeOffset.UtcNow.ToUnixTimeMilliseconds())?"已经看见你 · 尽情挥动双手":"等待肩膀和双手进入画面 · "+client.Status,small);
