    ...
    public sealed partial class ArenaController
    {
        ...
        void CaptureGuidedProof()
        {
            ...
            string key=photo.Active?"photo-"+photo.Stage+(photo.Stage==PhotoStage.Framing?(photo.HasLivePerson?"-live":"-preparing"):""):
                battle.Phase==GamePhase.Battle?(world.Closeup.Active?"beam-closeup":battle.Enemy==EnemyPhase.Windup?"guard-guide":battle.Energy>=15?"beam-guide":battle.Punches>2?"battle":"battle-entry"):
        ...
        IEnumerator SaveGuidedProof(string key)
        {
            ...
            string folder=Path.Combine(System.Environment.CurrentDirectory,"artifacts/guided-arcade/native");Directory.CreateDirectory(folder);
            ...
            Debug.Log("[GuidedProof] screenshot="+key+" source=synthetic");
        }
    }
}
