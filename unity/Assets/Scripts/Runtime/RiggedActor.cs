...
namespace UltramanGame.Runtime
{
    ...
    public sealed class RiggedActor
    {
        ...
        RiggedActor(string name,string path,GameObject prefab,Vector3 position,Vector3 opponent,bool isMonster)
        {
            ...
            float size=(monster?3.35f:3.45f)/Mathf.Max(.01f,bounds.size.y);
            placement.localScale=Vector3.one*size;
            placement.localPosition=-new Vector3(bounds.center.x,bounds.min.y,bounds.center.z)*size;
            ...
            foreach(var renderer in renderers)
            {
                ...
                for(int i=0;i<mapped.Length;i++)
                {
                    ...
                    {mat=Surface(key,texture);materialCache[key]=mat;materials.Add(mat);}
                    mapped[i]=mat;
                }
        ...
        static Material Surface(string name,Texture2D texture)
        {
            ...
            if(name.Contains("Suit")) {mat.mainTexture=texture;mat.color=Color.white;mat.SetFloat("_Metallic",.15f);mat.SetFloat("_Glossiness",.42f);}
        ...
        public void Update(Battle state,float dt,float time,int preview=-1)
        {
            ...
            if(monster)
            {
                ...
                else if(state.Phase==GamePhase.Battle&&hitAge<.4f) {next="Hurt";sample=hitAge;Frame=heavyHit?6:5;}
                else if(state.Phase==GamePhase.Battle&&state.Enemy==EnemyPhase.Windup) {next="Windup";sample=state.EnemyAge/state.WarningDuration*clips[next].length;Frame=3;travel=AnimatedActor.MonsterAdvance(state);}
            }
            ...
            else if(state.Phase==GamePhase.Battle)
            {
                ...
                {next=state.Action==HeroAction.LeftPunch?"LeftPunch":"RightPunch";sample=state.ActionAge;Frame=sample<.07f?1:2;travel=AnimatedActor.Strike(sample)*1.45f;}
